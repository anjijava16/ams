     ...
     <link href="<%=basePath%>/css/common/base.css" rel="stylesheet"/>
		...
		<script >
			...
			function _delete(ids){
				var index = layer.confirm("确定这"+ ids.length +"个基础车型？",function(){
					var load = layer.load();
			...
			function addCarModel(){
				...
				<!--loding-->
				var load = layer.load();
				$.post('<%=basePath %>/carModel/addCarModel',{carbrand:carbrand,carmodel:carmodel,caryear:caryear,carprice:carprice},function(result){
					layer.close(load);
					if(result && result.status != 200){
			...
			*/
			function editById(id){
				var load = layer.load();
				$.post("<%=basePath%>/carModel/editCarModelById",{id:id},function(result){
					layer.close(load);
					if(result){
						var html =[];
			...
			function updateCarModel(){
				...
				<!--loding-->
				var load = layer.load();
				$.post('<%=basePath%>/carModel/updateCarModel',{id:$('#selectCarModelId').val(),carbrand:carbrand,carmodel:carmodel,caryear:caryear,carprice:carprice},function(result){
					layer.close(load);
					if(result && result.status != 200){
						return layer.msg(result.message,so.default),!1;
					}
				...
				<form method="post" action="" id="formId" class="form-inline">
						<div clss="well">
					     ...
					     <span class=""> 
                             ...
                             <a class="btn btn-success" onclick="$('#addCarModel').modal();">添加基础车型</a>
					...
					<c:forEach items="${page.list}" var="it">
                        ...
                        <c:if test="${page.list==null || fn:length(page.list) < 1}">
							<tr>
								<td class="text-center danger" colspan="7">没有找到基础车型</td>
							</tr>
						</c:if>
					</table>
					    <c:if test="${page.list!=null && fn:length(page.list) > 0}">
			  ...
			  <div class="modal-dialog modal-sm" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        ...
			        <h4 class="modal-title" id="addCarModelLabel">添加基础车型</h4>
			      </div>
			...
			<div class="modal fade bs-example-modal-sm"  id="editCarModel" tabindex="-1" role="dialog" aria-labelledby="editCarModelLabel">
			  <div class="modal-dialog modal-sm" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			        <h4 class="modal-title" id="editCarModelLabel">编辑基础车型</h4>
			      </div>
			      <div class="modal-body">
