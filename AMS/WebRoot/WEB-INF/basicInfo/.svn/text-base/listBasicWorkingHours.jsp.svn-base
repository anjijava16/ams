...
<html lang="zh-cn">
	<head>
		<meta charset="utf-8" />
		<title>系统管理 —数据字典-基础标准工时管理</title>
     ...
     <link href="<%=basePath%>/css/common/base.css" rel="stylesheet"/>
		...
		<script >
			...
			function _delete(ids){
				var index = layer.confirm("确定这"+ ids.length +"个基础标准工时？",function(){
					var load = layer.load();
					$.post('<%=basePath %>/workingHours/deleteWorkingHoursById',{ids:ids.join(',')},function(result){
						layer.close(load);
				...
				<form method="post" action="" id="formId" class="form-inline">
						<div clss="well">
					     ...
					     <span class=""> 
                             ...
                             <a class="btn btn-success" href="<%=basePath%>/workingHours/toAddWorkingHours">添加基础标准工时</a>
					...
					<c:forEach items="${page.list}" var="it">
								...
								<td>${it.taskproject}</td>
									...
									<td>${it.price}</td>		<td>
										...
										<i class="glyphicon glyphicon-remove"></i><a href="javascript:_delete([${it.id}]);">删除</a> <i class="glyphicon glyphicon-edit"></i><a href="<%=basePath%>/workingHours/editWorkingHoursById?id=${it.id}">编辑</a>
                        ...
                        <c:if test="${page.list==null || fn:length(page.list) < 1}">
							<tr>
								<td class="text-center danger" colspan="12">没有找到基础标准工时数据</td>
							</tr>
						</c:if>
					</table>
					    <c:if test="${page.list!=null && fn:length(page.list) > 0}">
