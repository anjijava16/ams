...
<html lang="zh-cn">
	<head>
		<meta charset="utf-8" />
		<title>系统管理 —数据字典-基础工位分类管理</title>
     ...
     <link href="<%=basePath%>/css/common/base.css" rel="stylesheet"/>
		...
		<script >
			...
			function _delete(ids){
				var index = layer.confirm("确定这"+ ids.length +"个基础工位分类？",function(){
					var load = layer.load();
					$.post('<%=basePath %>/workplaceType/deleteWorkplaceTypeById',{ids:ids.join(',')},function(result){
						layer.close(load);
			...
			function addWorkplaceType(){
				...
				<!--loding-->
				var load = layer.load();
				$.post('<%=basePath %>/workplaceType/addWorkplaceType',{place_parent_name:place_parent_name,place_name:place_name},function(result){
					layer.close(load);
					if(result && result.status != 200){
			...
			*/
			function editById(id){
				var load = layer.load();
				$.post("<%=basePath%>/workplaceType/editWorkplaceTypeById",{id:id},function(result){
					layer.close(load);
					if(result){
						var html =[];
			...
			function updateWorkplaceType(){
				...
				<!--loding-->
				var load = layer.load();
				$.post('<%=basePath%>/workplaceType/updateWorkplaceType',{id:$('#selectWorkplaceTypeId').val(),place_parent_name:place_parent_name,place_name:place_name},function(result){
					layer.close(load);
					if(result && result.status != 200){
						return layer.msg(result.message,so.default),!1;
					}
				...
				<form method="post" action="" id="formId" class="form-inline">
						<div clss="well">
					     ...
					     <span class=""> 
                             ...
                             <a class="btn btn-success" onclick="$('#addWorkplaceType').modal();">添加基础工位类型</a>
					...
					<c:forEach items="${page.list}" var="it">
								<tr>
									...
									<td>${it.place_name}</td><td>
										...
										<i class="glyphicon glyphicon-remove"></i><a href="javascript:_delete([${it.id}]);">删除</a> <i class="glyphicon glyphicon-edit"></i><a href="javascript:editById(${it.id});">编辑</a>
                        ...
                        <c:if test="${page.list==null || fn:length(page.list) < 1}">
							<tr>
								<td class="text-center danger" colspan="4">没有找到基础工位分类数据</td>
							</tr>
						</c:if>
					</table>
					    <c:if test="${page.list!=null && fn:length(page.list) > 0}">
			  ...
			  <div class="modal-dialog modal-sm" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        ...
			        <h4 class="modal-title" id="addWorkplaceTypeLabel">添加基础车型分类数据</h4>
			      </div>
			...
			<div class="modal fade bs-example-modal-sm"  id="editWorkplaceType" tabindex="-1" role="dialog" aria-labelledby="editWorkplaceTypeLabel">
			  <div class="modal-dialog modal-sm" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			        <h4 class="modal-title" id="editWorkplaceTypeLabel">编辑基础工位分类数据</h4>
			      </div>
			      <div class="modal-body">
