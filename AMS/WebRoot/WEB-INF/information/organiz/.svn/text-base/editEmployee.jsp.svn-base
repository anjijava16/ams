...
<html lang="zh-cn">
	<head>
		<meta charset="utf-8" />
		<title>钣喷中心信息管理 —组织机构-员工编辑</title>
   ...
   <li>
         ...
         <li>
			...
			<div class="col-md-10">
					<h2>员工添加</h2>
					<hr>
				<form id="formId" enctype="multipart/form-data" action="<%=basePath%>/organiz/updateEmployee" method="post" class="form-horizontal">                       
                       <div class="form-group">
<label for="e_name" class="col-sm-2 control-label">员工姓名</label>
						 <div class="col-sm-5"> <input type="text" class="form-control" name="e_name" id="e_name" placeholder="请输入员工姓名" value="${employee.e_name}"/></div>
</div>
 <div class="form-group">
 ...
 <option value="1" <c:if test="${employee.e_id_type == '1'}">selected</c:if>>身份证</option>
  <option value="2"  <c:if test="${employee.e_id_type == '2'}">selected</c:if>>驾驶证</option>
   <option value="3"  <c:if test="${employee.e_id_type == '3'}">selected</c:if>>军官证</option>
    <option value="4"  <c:if test="${employee.e_id_type == '4'}">selected</c:if>>护照</option>                                                                </select></div>
</div>
    <div class="form-group">
						    <label for="e_id_num" class="col-sm-2 control-label">证件号码</label>
						 <div class="col-sm-5"> <input type="text" class="form-control" name="e_id_num" id="e_id_num" placeholder="请输入证件号码" value="${employee.e_id_num}"/></div></div>
                         	<div class="form-group">
                    ...
                    <input class="form-control" size="16" type="text" value="${employee.e_birth}" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
					<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
				<input type="hidden" id="e_birth" value="${employee.e_birth}"  name="e_birth"/><br/>
            </div>
				 <div class="form-group">
      ...
      <option value="">请选择...</option>
 <option value="1"  <c:if test="${employee.e_sex == '1'}">selected</c:if>>男</option>
  <option value="2"  <c:if test="${employee.e_sex == '2'}">selected</c:if>>女</option>
                                                                  </select></div>
</div>
 <div class="form-group">
      ...
      <option value="">请选择...</option>
 <option value="1"  <c:if test="${employee.e_marry == '1'}">selected</c:if>>未婚</option>
  <option value="2"  <c:if test="${employee.e_marry == '2'}">selected</c:if>>已婚</option>
                                                                  </select></div>
</div>
<div class="form-group">
<label for="e_tel" class="col-sm-2 control-label">电话</label>
						 <div class="col-sm-5"> <input type="text" class="form-control" name="e_tel" id="e_tel" placeholder="请输入电话" value="${employee.e_tel}"/></div>
</div>
 <div class="form-group">
						  <label for="e_educate" class="col-sm-2 control-label">学历</label><div class="col-sm-5">
                                <input type="text" class="form-control" name="e_educate" id="e_educate" placeholder="请输入学历" value="${employee.e_educate}"/></div>
</div>
 <div class="form-group">
      ...
      <option value="">请选择...</option>
 <option value="1" <c:if test="${employee.e_political == '1'}">selected</c:if>>群众</option>
  <option value="2"  <c:if test="${employee.e_political == '2'}">selected</c:if>>团员</option>
   <option value="3"  <c:if test="${employee.e_political == '3'}">selected</c:if>>党员</option>
                                                                  </select></div>
</div>
<div class="form-group">
                    ...
                    <input class="form-control" size="16" type="text" value="${employee.e_entry_time}" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
					<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
				<input type="hidden" id="e_entry_time" value="${employee.e_entry_time}"  name="e_entry_time"/><br/>
            </div>
            <div class="form-group">
                    ...
                    <input class="form-control" size="16" type="text" value="${employee.e_leave_time}" readonly>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
					<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
				<input type="hidden" id="e_leave_time" value="${employee.e_leave_time}"  name="e_leave_time"/><br/>
            </div>
             <div class="form-group">
      ...
      <option value="">请选择...</option>             
                                        ...
                                        <c:forEach items="${listDepartment }" var="it">
                                         <c:choose>
      <c:when test="${it.id eq employee.d_id}">
       <option value="${it.id}" selected>${it.d_name}</option>
      </c:when>
      
      <c:otherwise> <option value="${it.id}">${it.d_name}</option></c:otherwise>
        </c:choose>
 ...
 <div class="form-group">
      ...
      <option value="">请选择...</option>             
         ...
         <c:choose>
      <c:when test="${it.id eq employee.s_id}">
       <option value="${it.id}" selected>${it.s_name}</option>
      </c:when>
      
      <c:otherwise> <option value="${it.id}">${it.s_name}</option></c:otherwise>
      </c:choose>
                             </c:forEach>                                       </select></div>
</div>
<div class="form-group">
      ...
      <option value="">请选择...</option>             
         ...
         <c:choose>
      <c:when test="${it.id eq employee.p_id}">
       <option value="${it.id}" selected>${it.p_name}</option>
      </c:when>
      
      <c:otherwise> <option value="${it.id}">${it.p_name}</option></c:otherwise>
      </c:choose>
                             </c:forEach>                                       </select></div>
</div>
<div class="form-group">
<label for="seq" class="col-sm-2 control-label">排序</label>
						 <div class="col-sm-5"> <input type="text" class="form-control" name="seq" id="seq" placeholder="请输入排序" value="${employee.seq}"/></div>
</div>
  <div class="form-group">
						   <label for="e_remarks" class="col-sm-2 control-label">备注</label><div class="col-sm-5">
						 <textarea name="e_remarks"   id="e_remarks" class="form-control">${employee.e_remarks}</textarea>
						</div>
                          </div>
	    <div class="form-actions">
                             ...
                             <input type="hidden" name="centerid" value="${employee.centerid}">
