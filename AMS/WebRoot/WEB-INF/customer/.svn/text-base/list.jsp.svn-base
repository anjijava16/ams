...
<body data-target="#one" data-spy="scroll">
	...
	<div class="container"
		...
		<jsp:include page="menu.jsp" flush="true"></jsp:include>
			<div class="col-md-10">
				<h2>客户信息列表</h2>
				<hr>
				<form method="post" action="/AMS/customer/list" id="formId" class="form-inline">
					<div clss="well">
						<div class="form-group">
							<input type="text" class="form-control" style="width: 300px;"
								value="${findContent}" name="findContent" id="findContent"
								placeholder="输入用户名 / 手机号/ 证件号">
						</div>
						<span class="">
							<button type="submit" class="btn btn-primary">查询</button>
							<a class="btn btn-success" href="/AMS/customer/add">新增客户</a>
						</span>
					</div>
					</form>
					<hr>
					<table class="table table-striped table-bordered">
						...
						<c:if test="${page.list!=null && fn:length(page.list) > 0}">
							<c:forEach items="${page.list}" var="it">
								<tr>
									...
									<td>${it.id_num}</td>
									<td>
									<a href="<%=basePath %>/customer/detail?id=${it.id}">查看/编辑</a>
						...
						<c:if test="${page.list==null || fn:length(page.list) < 1}">
							<tr>
								<td class="text-center danger" colspan="4">没有找到客户信息</td>
							</tr>
						</c:if>
					</table>
					<c:if test="${page.list!=null && fn:length(page.list) > 0}">
	...
	<script>
        function deleteCus(id){
			var index = layer.confirm("确定要删除这个客户以及关联的车辆和联系人吗？",function(){
			var load = layer.load();
