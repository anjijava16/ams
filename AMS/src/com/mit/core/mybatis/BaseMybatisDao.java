...
import com.mit.core.mybatis.page.OracleDialect;
...
public class BaseMybatisDao<T> extends SqlSessionDaoSupport {
	...
	public Pagination findByPageBySqlId(String sqlId,
		...
		params.put("firstRow", offset);
		params.put("lastRow", offset+pageSize);
		BoundSql boundSql = c.getMappedStatement(sqlId).getBoundSql(params);
		String sqlcode = boundSql.getSql();//返回SQL代码	
		...
		if (StringUtils.isBlank(sqlId)) {
			...
			countMap.remove("firstRow");//去掉，分页的参数。
			countMap.remove("lastRow");//去掉，分页的参数。
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public List findNoPageList(String sqlId, Map<String, Object> params) {
		sqlId = String.format("%s.%s", NAMESPACE,sqlId) ;
